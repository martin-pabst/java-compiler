...
export class HitPolygonStore {
    ...
    public static getPolygonForTexture(name: string, index: number, spriteHelper: SpriteClass, sprite: PIXI.Sprite): Punkt[] {
        ...
        if (polygon == null) {
            ...
            polygon = HitPolygonStore.getPolygon(sprite, spriteHelper.world);
    ...
    private static getPolygon(sprite: PIXI.Sprite, worldHelper: IWorld): Punkt[] {
        let renderer = <PIXI.Renderer>worldHelper.app.renderer;
        let pixels = renderer.extract.pixels(sprite).pixels;
        let w = sprite.width;
        let h = sprite.height;
    ...
    private static isTransparent(x: number, y: number, width: number, height: number, pixels: Uint8ClampedArray) {
